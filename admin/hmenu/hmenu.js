 ...
 *
 *           LICENSEE: Dynarch.com user: spawn
 *        License key: linkware-2068542
 *      Purchase date: Wed Oct 19 12:48:53 2005 GMT
...
DynarchMenu._documentMouseDown=function(ev){ev||(ev=window.event);
...
DynarchMenu._documentMouseUp=function(ev){ev||(ev=window.event);
if(DynarchMenu._msupTimeout)return false;
var menu=_dynarch_top.DynarchMenu._C,el,info;
if(menu){el=is_ie?ev.srcElement:ev.target;
for(;
el&&!(el.__msh_is_dynarch_menu&&(info=el.__msh_info));
el=el.parentNode);
if(!el)_dynarch_top.DynarchMenu._closeOtherMenus(null);
else if(info&&info.exec)info.exec(ev);
}_dynarch_top.DynarchMenu._C=null;
_dynarch_top.DynarchMenu._activeItem=null;
};
DynarchMenu._documentMouseOver=function(ev){var menu=_dynarch_top.DynarchMenu._C,el,tmout;
if(menu&&menu.config.electric){ev||(ev=window.event);
el=is_ie?ev.srcElement:ev.target;
for(;
el&&!el.__msh_is_dynarch_menu;
el=el.parentNode);
if(!el||el===document.body){tmout=menu.config.electric;
...
DynarchMenu._documentKeyPress=function(ev){ev||(ev=window.event);
DynarchMenu._forAllMenus(function(menu){var tmp=menu._activePopup,item=tmp?tmp.active_item:null,kmap;
function do_27(){if(tmp){tmp.close(true,true);
if(item)item.mouseout();
if(tmp.base||(tmp.parent.base&&tmp.config.context)){tmp.resetActive();
...
if(item.action&&!item.submenu)item.exec(ev);
DynarchMenu._stopEvent(ev);
};
function do_ud(up){if(tmp){if(!item)item=up?tmp.getFirstItem(item):tmp.getLastItem(item);
...
DynarchMenu.JSAction.prototype.exec=function(ev){
	var retval=false;
	eval(this.js);
	return retval;
};
...
DynarchMenu.LinkAction.prototype.exec=function(ev){
	if(ev.ctrlKey) {
...
DynarchMenu.DefaultAction.prototype.exec=function(ev){
...
DynarchMenu.MenuItem.prototype._exec=function(ev){if(!this.disabled&&!this.separator&&this.action&&!this.action.exec(ev)){DynarchMenu._class(this.element,DynarchMenu._RE_AH);
...
DynarchMenu.MenuItem.prototype.exec=function(ev){var step=this.menu.config.blink;
if((this.submenu&&!this.action.explicit)||this.html_popup||!step)return this._exec(ev);
var self=this;
...
DynarchMenu.getWinSize=function(){if(is_gecko){if(document.documentElement.clientWidth)return{x:document.documentElement.clientWidth,y:document.documentElement.clientHeight};
else return{x:window.innerWidth,y:window.innerHeight};
...
DynarchMenu._nfo={product:"hmenu-2.8.1",licensee:"Dynarch.com user: spawn",license_key:"linkware-2068542",purchase_date:"Wed Oct 19 12:48:53 2005 GMT",license_type:"linkware"};
