...
function fileQueueError(file, errorCode, message) {
	try {
		...
		case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
			progress.setStatus("Невозможно загрузить файл размером 0 байт.");
		...
		default:
			if (file !== null) {
				progress.setStatus("Неопределенная ошибка");
			}
...
function queueComplete(numFilesUploaded) {
	...
	status.innerHTML = "Загружено " + numFilesUploaded + " файл" + (numFilesUploaded === 1 ? "" : "ов");
}
